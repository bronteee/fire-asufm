# This architecture is based on https://github.com/sheikhazhanmohammed/SADMA/blob/main/model.py
...
class ResidualBlock(nn.Module):
    def __init__(self, inputChannel, outputChannel, stride=1, downsample=None, activation='relu'):
...
class BasicDownSample(nn.Module):
    def __init__(self, inputChannel, outputChannel, activation='leakyrelu'):
        super().__init__()
...
class AttentionUNet(nn.Module):
    def __init__(self, inputChannel, outputChannel, activation='leakyrelu'):
        super().__init__()
        self.n_channels = inputChannel
        self.n_classes = outputChannel
        self.downsample1 = DownSampleWithAttention(inputChannel, 32, activation=activation)
        self.downsample2 = DownSampleWithAttention(32, 64, activation=activation)
        self.downsample3 = DownSampleWithAttention(64, 128, activation=activation)
        self.downsample4 = DownSampleWithAttention(128, 256, activation=activation)
        self.downsample5 = DownSampleWithAttention(256, 512, activation=activation)

        self.upsample1 = UpSampleWithAttention(512, 256, activation=activation)
        self.upsample2 = UpSampleWithAttention(512, 128, activation=activation)
        self.upsample3 = UpSampleWithAttention(256, 64, activation=activation)
        self.upsample4 = UpSampleWithAttention(128, 32, activation=activation)
        self.upsample5 = UpSampleWithAttention(64, 32, activation=activation)
...
if __name__ == '__main__':
    # model = AttentionUNet(4, 2)
    model = ResidualAttentionUNet(4, 2)
    print(model)
