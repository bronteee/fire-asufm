...
class Decoder(nn.Module):
    def __init__(
        self, depth=[2, 2, 2, 2], dim=[112, 72, 40, 24], block_type=None, kernel_size=3
    ) -> None:
        super().__init__()
        self.depth = depth
        self.dim = dim
    ...
    def _build_decode_layer(self, dim, depth, kernel_size):
        ...
        for i in range(1, len(dim)):
            module = [
                self.block_type(dim[i], dim[i], dim[i], kernel_size)
            ...
            proj_layers.append(
                nn.Sequential(
                    nn.Conv2d(dim[i - 1], dim[i], 1, 1), norm_layer(dim[i]), nn.GELU()
                )
            )
